  ...
  while (done != 1) {
    ...
    if (tag == 1) 
    {
      ...
      Result <- list(result=Score, SampleFileName = task$SampleFileName, PhageFileName = task$PhageFileName, 
                     AlignmentScore = AlignmentScoreBoundary)
...
ScoreStart = 14
ScoreEnd = 19
tasks = GenerateTasks(Samples, Phages, ScoreStart, ScoreEnd)
ExprMatricies = GenerateEmptyMatricies(length(Samples), length(Phages), ScoreStart, ScoreEnd)
...
{ 
  ...
  if (tag == 1) { 
    ...
    print(paste(which(Samples == message$SampleFileName), which(Phages == message$PhageFileName), "Score ", message$AlignmentScore))
    Expression_Matrix = ExprMatricies[[message$AlignmentScore - ScoreStart + 1]][["ExprMatrix"]]
    Expression_Matrix[which(Samples == message$SampleFileName), which(Phages == message$PhageFileName)] = message$result
    ExprMatricies[[message$AlignmentScore - ScoreStart + 1]][["ExprMatrix"]] = Expression_Matrix
  } 
  else if (tag == 3) { 
...
for (i in 1:length(ExprMatricies))
{
  Expression_Matrix = ExprMatricies[[i]][["ExprMatrix"]]
  
  colnames(Expression_Matrix) <- PhageNames
  rownames(Expression_Matrix) = SampleNames
  
  ResDf = as.data.frame(Expression_Matrix)
  write.table(ResDf,file = paste("c:/CRISPr/ThermusToPhage/data/res_score", ExprMatricies[[i]][["AlignmentScore"]], ".txt", sep = ""))
